...
class AssRunSummary:
    ...
    converted_otf: int
    skipped_otf: int
    deleted_source_ass: int
    cleaned_work_dir: bool
    duration_s: float
    outputs: list[str]
    failures: list[str]
...
def run_ass_pipeline(settings: AssSettings) -> AssRunSummary:
    ...
    cleaned_work_dir = False
    deleted_source_ass = 0
    reset_dir(work_dir)
    try:
        ...
        if not pending:
            ...
            if settings.cleanup_work_dir_on_success and work_dir.exists():
                shutil.rmtree(work_dir)
                cleaned_work_dir = True
            return AssRunSummary(str(target_dir), len(scan.ass_files), 0, len(scan.ass_files), 0, len(scan.archives), 0, 0, 0, deleted_source_ass, cleaned_work_dir, duration, outputs, [])
        ...
        for ass_file in pending:
            try:
                ...
                processed += 1
                successful_sources.append(ass_file)
        ...
        if failed == 0 and settings.delete_source_ass_on_success:
            for ass_file in successful_sources:
                try:
                    ass_file.unlink()
                    deleted_source_ass += 1
                    logger.info('🗑️ 删除原始 ASS: %s', ass_file)
                except Exception as exc:
                    logger.warning('⚠️ 删除原始 ASS 失败: %s (%s)', ass_file, exc)
        duration = time.monotonic() - started
        if failed == 0 and settings.cleanup_work_dir_on_success and work_dir.exists():
            shutil.rmtree(work_dir)
            cleaned_work_dir = True
        return AssRunSummary(
            ...
            deleted_source_ass=deleted_source_ass,
            cleaned_work_dir=cleaned_work_dir,
            duration_s=duration,
            outputs=outputs,
            failures=failures,
        )
    except Exception:
        if settings.cleanup_work_dir_on_failure and work_dir.exists():
            shutil.rmtree(work_dir)
            cleaned_work_dir = True
        raise
